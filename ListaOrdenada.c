...
int tamanhoEmBytes(LISTA* l) {
  int tamanhoBytes = sizeof(REGISTRO) * l -> tamanhoLista;
  return sizeof(LISTA) + tamanhoBytes;
} /* tamanhoEmBytes */
...
bool excluirElemLista(LISTA* l, TIPOCHAVE ch) { 
  ...
  l->nroElem--;
  if(l -> tamanhoLista / 4 >= l -> nroElem && l -> tamanhoLista > 50){
    reduzirTamanho(l);
  }
  return true;
} /* excluirElemLista */
...
void aumentarTamanho(LISTA* l){
  l -> tamanhoLista = l -> tamanhoLista * 2;
  int * novaLista = realloc(l -> A, l -> tamanhoLista * sizeof(REGISTRO));
  l -> A = novaLista;
}
